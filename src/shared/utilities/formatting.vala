...
namespace Rpy.Utils {
    string format_number (int64 number, string skeleton) {
        ...
        Icu.String ustring = uresult.to_string (ref ec);
        string     @string = ustring.to_utf8 (-1, ref ec);
    ...
    string format_relative_date (DateTime date) {
        ...
        DateTime local_date = date.to_local ();
        TimeSpan difference = now.difference (local_date);
        ...
        if (difference < TimeSpan.DAY) {
            int day_hour  = now.get_hour ();
            var threshold = 24.0 * 0.2;
        ...
        if (difference < TimeSpan.DAY * 7) {
            int week_day  = now.get_day_of_week ();
            var threshold = 7.0 * 0.2;

            int days = week_day - local_date.get_day_of_week ();
        ...
        if (difference < TimeSpan.DAY * 30) {
            int month_week = get_week_of_month (now);
            var threshold  = get_weeks_of_month (now) * 0.2;

            int weeks = month_week - get_week_of_month (local_date);
        ...
        if (difference < TimeSpan.DAY * 365) {
            int year_month = now.get_month ();
            var threshold  = 12.0 * 0.2;

            int months = year_month - local_date.get_month ();
        ...
        int years = now.get_year () - local_date.get_year ();
    ...
    string format_timespan (TimeSpan timespan) {
        ...
        string format          = "%02" + int64.FORMAT;
        string minutes_seconds = (format + "∶" + format).printf (minutes, seconds);
    ...
    int get_week_of_month (DateTime date) {
        ...
        int first_day_missing_days = first_day.get_day_of_week ();
    ...
    int get_weeks_of_month (DateTime date) {
        var first_day     = new DateTime.local (date.get_year (), date.get_month (), 1, 0, 0, 0);
        DateTime last_day = first_day.add_months (1).add_days (-1);

        int first_week_missing_days = first_day.get_day_of_week ();
        int last_week_missing_days  = 7 - last_day.get_day_of_week ();

        int month_with_missing_days = last_day.get_day_of_month () +
